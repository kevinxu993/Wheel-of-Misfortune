...
char **prune_word_list(char **words, int len, int *words_remaining) {
    int word_count = 0;
    char **cur_word_pt = words;
    while ((*cur_word_pt) != NULL) {
        if (strlen(*cur_word_pt) == len) {
            word_count++;
        }
        cur_word_pt += 1;
    }
    *words_remaining = word_count;
    ...
    // We got the number of words of length *len*
    char **res = malloc((word_count + 1) * sizeof(char*));
    int i = 0;
    cur_word_pt = words;
    while ((*cur_word_pt) != NULL) {
        if (strlen(*cur_word_pt) == len) {
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    while (fgets(buf, BUF_SIZE, stdin) != NULL) {
        buf[strlen(buf)-1] = '\0';
        if (is_numeric_string(buf)) {
            int length = strtol(buf, NULL, 10);
            int words_remaining;
            char **prune_res = prune_word_list(words, length, &words_remaining);
